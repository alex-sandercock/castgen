...
test_that("capture_diversity.Gmat works with a sample list", {
  ...
  result <- capture_diversity.Gmat(test_gmat, ploidy = 2, iterations = 2, sample_list = sample_list, save.result = FALSE, parallel=FALSE)
...
test_that("capture_diversity.Gmat works without a sample list", {
  result <- capture_diversity.Gmat(test_gmat, ploidy = 2, iterations = 2, save.result = FALSE, parallel=FALSE)
  expect_true(all(c("Individuals", "CI_Lower", "CI_Upper", "Iterations") %in% colnames(result)))
})

# Test with parallel processing
test_that("capture_diversity.Gmat works with parallel processing", {
  result <- capture_diversity.Gmat(test_gmat, ploidy = 2, iterations = 2, save.result = FALSE, parallel = TRUE)
  expect_true(all(c("Individuals", "CI_Lower", "CI_Upper", "Iterations") %in% colnames(result)))
})
...
test_that("capture_diversity.Gmat can save results", {
  ...
  result <- capture_diversity.Gmat(test_gmat, ploidy = 2, iterations = 2, save.result = TRUE, parallel=FALSE)
